<?php include 'header.php';?>
<?php include 'sidebar.php';?>
<div class="col-xs-8 col-md-10 col-xs-12 no-padding body-background">

    <div class="col-md-12 col-xs-12 no-padding head-color bottom-box body-background">
        <div class="head-p">
            <p>振替データ読込</p>
        </div>
        <div class="col-xs-12 no-padding">
            <p>銀行名一覧</p>
        </div>

        <div class="col-xs-12 no-padding">
            <div class="col-xs-5 no-padding table-scroll-2">
                <table class="table table-bordered col-xs-12">
                    <thead>
                    <tr class="bg-color">
                        <td></td>
                        <td class="td-style col-xs-5">銀行コード</td>
                        <td class="td-style col-xs-7">銀行名</td>
                    </tr>
                    <tr>
                        <td class="bg-color">1</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">2</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">3</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">4</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">5</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">6</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">7</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">8</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">9</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">10</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">11</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">12</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">13</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">13</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">14</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">15</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">16</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">17</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">18</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    <tr>
                        <td class="bg-color">19</td>

                        <td><input type="text" class="form-control tbl-input"></td>
                        <td><input type="text" class="form-control tbl-input"></td>
                    </tr>
                    </thead>
                </table>
            </div>

            <div class="col-xs-7">
                <div class="col-xs-12">
                    <div class="col-xs-2 no-padding border-margin-exp">
                        <p>銀行コード</p>
                    </div>
                    <div class="col-xs-4">
                        <input type="number" class="form-control">
                    </div>
                    <div class="col-xs-5">
                        <input type="text" class="form-control">
                    </div>

                </div>

                <div class="col-xs-12 border-margin">
                    <div class="col-xs-2 no-padding border-margin-exp">
                        <p>銀行区分</p>
                    </div>
                    <div class="col-xs-3">
                        <input type="number" class="form-control" value="1" disabled>
                    </div>
		    <div class="col-xs-2">
			1:直接取引分
		    </div>
		    <div class="col-xs-3">
			2:幹事銀行(振替代行分)
		    </div>
                </div>
                <div class="col-xs-12 border-margin">
                    <div class="col-xs-2 no-padding border-margin-exp">
                        <p>振替日</p>
                    </div>
                    <div class="col-xs-4">
                        <input type="date" class="form-control">
                    </div>


                </div>




            </div>

        </div>


    </div>


    <?php include 'footer.php';?>

</div>

</div>
</div>
</div>

</body>

</html>
